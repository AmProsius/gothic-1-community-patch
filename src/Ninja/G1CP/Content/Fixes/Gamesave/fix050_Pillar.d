...
const float G1CP_050_Pillar_PosNew[16] = {
...
func int G1CP_050_Pillar() {
    ...
    var int vobPtr; vobPtr = G1CP_050_PillarFind();
    if (vobPtr) {

        // Confirm exact position
        if (!MEM_CompareWords(vobPtr+60, _@(G1CP_050_Pillar_PosOld), 16)) {
            return FALSE;
        };

        // Confirm exact bounding box, in case the visual was replaced
        if (!MEM_CompareWords(vobPtr+124, _@(G1CP_050_Pillar_BBoxUp), 6))     // Upright
        && (!MEM_CompareWords(vobPtr+124, _@(G1CP_050_Pillar_BBoxDown), 6)) { // Fallen
            return FALSE;
        };
 ...
 */
func int G1CP_050_PillarRevert() {
    ...
    if (vobPtr) {
        // Revert state func
        var oCMobInter mob; mob = _^(vobPtr);
        if (Hlp_StrCmp(mob.onStateFuncName, "G1CP_050_PILLAR_STATEFUNC")) {
            mob.onStateFuncName = "";
        };
    ...
    const int oCMobInter__StopInteraction_End = 6816979; //0x6804D3
    RemoveHookF(oCMobInter__StopInteraction_End, 7, G1CP_050_Pillar_FixBbox);
 ...
 */
func void G1CP_050_Pillar_StateFunc_S1() {
    G1CP_ReportFuncToSpy();

    // Can't update the bounding box yet. Wait for complete stop
    const int oCMobInter__StopInteraction_End = 6816979; //0x6804D3
    if (G1CP_CheckBytes(oCMobInter__StopInteraction_End, "8B 8C 24 8C 00 00 00")) {
        HookEngineF(oCMobInter__StopInteraction_End, 7, G1CP_050_Pillar_FixBbox);
    };
};
func void G1CP_050_Pillar_FixBbox() {
    G1CP_ReportFuncToSpy();

    var int vobPtr; vobPtr = ESI;
    if (vobPtr != G1CP_050_PillarFind()) {
