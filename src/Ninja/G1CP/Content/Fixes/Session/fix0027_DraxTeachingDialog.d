 ...
 */
func int G1CP_0027_DraxTeachingDialog() {
    ...
    repeat(i, MEM_ArraySize(matches)); var int i;
        ...
        if (MEM_ReadByte(pos-5) == zPAR_TOK_PUSHINT) || (MEM_ReadByte(pos-5) == zPAR_TOK_PUSHVAR) {
            ...
            MEMINT_OFTokPar(zPAR_TOK_CALL, MEM_GetFuncOffset(G1CP_0027_SetVar));
            applied1 = TRUE;
        };
    end;
    ...
    if (applied1) {
        ...
        var int createNewId; createNewId = MEM_GetFuncId(G1CP_0027_CreateTopic);
    ...
    if (applied2) {
        var int logEntryOld; logEntryOld = G1CP_GetFuncId("B_LogEntry", "void|string|string");
        var int logEntryNew; logEntryNew = MEM_GetFuncId(G1CP_0027_TopicEntry);
        if (logEntryOld != -1) {
 ...
 */
func void G1CP_0027_CreateTopic(var string topic, var int section) {
    G1CP_ReportFuncToSpy();
 ...
 */
func void G1CP_0027_TopicEntry(var string topic, var string entry) {
    G1CP_ReportFuncToSpy();
