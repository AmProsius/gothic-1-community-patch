 ...
 */
func int G1CP_0021_FletcherClosedQuest() {
    ...
    var int createNewId; createNewId = MEM_GetFuncId(G1CP_0021_CreateTopic);
    var int statusNewId; statusNewId = MEM_GetFuncId(G1CP_0021_SetTopicStatus);
 ...
 */
func int G1CP_0021_AllowTopicCall(var string topic) {
 ...
 */
func void G1CP_0021_CreateTopic(var string topic, var int section) {
    G1CP_ReportFuncToSpy();

    // Define possibly missing symbols locally
    const int LOG_MISSION = 0;

    if (G1CP_0021_AllowTopicCall(topic)) || (section != LOG_MISSION) {
        Log_CreateTopic(topic, section);
    };
};

/*
 * Wrapper function for G1CP_0021_SetTopicStatus
 */
func void G1CP_0021_SetTopicStatus(var string topic, var int newStatus) {
    G1CP_ReportFuncToSpy();

    // Define possibly missing symbols locally
    const int LOG_RUNNING = 1;

    if (G1CP_0021_AllowTopicCall(topic)) || (newStatus != LOG_RUNNING) {
